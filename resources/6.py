...
# 当前选中的地皮类型
current_tile = None
current_tile_size = 50  # 初始化为缩小后的大小
            ...
            if event.button == 1:  # 左键点击
                for name, rect in buttons.items():
                    if rect.collidepoint(event.pos):
                        current_tile = name
                        current_tile_size = 50  # 重置为初始大小
                        break
                else:
                    if current_tile:  # 在地图上放置地皮
                        mouse_x, mouse_y = event.pos
                        tile_rect = pygame.Rect(mouse_x + camera_pos[0] - current_tile_size // 2, 
                                                mouse_y + camera_pos[1] - current_tile_size // 2, 
