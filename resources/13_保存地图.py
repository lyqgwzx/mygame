...
def save_map(filename):
    ...
    message_display_time = pygame.time.get_ticks()  # 获取当前时间毫秒数# 初始化 Pygame
...
def generate_buttons():
    global buttons, button_width, button_height
    buttons = {}
    y_offset = 50 + scroll_offset if show_ui else -button_height - 100  # 隐藏时将按钮移出屏幕
    for idx, name in enumerate(tile_images.keys()):
        rect = pygame.Rect(10, y_offset, button_width, button_height)
        buttons[name] = rect
        y_offset += button_height + button_margin
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                ...
                if show_ui:  # 仅当界面信息显示时处理按钮点击
                    for name, rect in buttons.items():
                        if rect.collidepoint(event.pos):
                            ...
                            selected_tile = None
                            ui_clicked = True  # 标记UI按钮被点击
                            break
                if not ui_clicked:  # 如果没有点击UI按钮，处理地图放置逻辑
            ...
            if event.key == pygame.K_h:
                ...
                generate_buttons()  # 每次切换显示状态时，重新生成按钮
            if current_tile:
                # 检测方向键以调整预览位置
