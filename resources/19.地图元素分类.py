...
# 确保捕获 TEXTINPUT 事件
pygame.event.set_allowed([pygame.KEYDOWN, pygame.KEYUP, pygame.TEXTINPUT, pygame.QUIT, pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP, pygame.MOUSEMOTION])
...
def load_scaled_images(image_path):
    ...
    for scale in sizes:
        if scale == 1.0:
            # 直接使用原始图像，避免重复缩放
            pygame_image = pygame.image.fromstring(original_image.tobytes(), original_image.size, original_image.mode).convert_alpha()
...
def generate_buttons():
    ...
    for category_name, elements in tile_images.items():
        # 添加类别标签
        category_rect = pygame.Rect(x_offset + scroll_bar_width, y_offset, screen_width // 2 - 2 * button_margin - scroll_bar_width, 30)
...
alignment_menu_rect = pygame.Rect(alignment_button_rect.x, alignment_button_rect.y - len(alignment_modes) * 30 - 10, 150, len(alignment_modes) * 30)
...
save_prompt_dont_save_button_rect = pygame.Rect(save_prompt_dialog_rect.centerx - 40, save_prompt_dialog_rect.bottom - 60, 80, 40)
save_prompt_cancel_button_rect = pygame.Rect(save_prompt_dialog_rect.right - 100, save_prompt_dialog_rect.bottom - 60, 80, 40)
...
pending_new_map = False  # 标志保存完成后是否需要新建地图
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if show_save_prompt_dialog:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if save_prompt_close_button_rect.collidepoint(event.pos) or save_prompt_cancel_button_rect.collidepoint(event.pos):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # 左键点击
                    ...
                    if not ui_clicked and mouse_in_map:
                        if current_tile:  # 在地图上放置地皮
                            ...
                            # 获取图片尺寸
                            image_width = tile_images[current_category][current_tile][current_scale][rotation_angle].get_width()
                            image_height = tile_images[current_category][current_tile][current_scale][rotation_angle].get_height()
                            
                            # 获取对齐偏移
                            alignment_offset_x, alignment_offset_y = get_alignment_offset(current_alignment_mode, image_width, image_height)
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_s and (keys[pygame.K_LCTRL] or keys[pygame.K_RCTRL] or keys[pygame.K_LMETA] or keys[pygame.K_RMETA]):
    ...
    if alignment_menu_visible:
        ...
        for idx, mode in enumerate(alignment_modes):
            mode_rect = pygame.Rect(alignment_menu_rect.x, alignment_menu_rect.y + idx * 30, alignment_menu_rect.width, 30)
    ...
    for tile in map_tiles:
        ...
        # 使用预加载的旋转图像
        scaled_thumb_image = pygame.transform.smoothscale(tile_images[tile["category"]][tile["tile"]][tile["scale"]][tile["angle"]], (thumb_rect.width, thumb_rect.height))
    ...
    if not current_tile:
        if hovered_tiles:
            info_text = "当前元素：" + " -> ".join([f"{tile['category']}/{tile['tile']}" for tile in hovered_tiles[::-1]])
        else:
            info_text = "当前元素：无"
