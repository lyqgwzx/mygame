...
while running:
            ...
            if event.button == 1:  # 左键点击
                for name, rect in buttons.items():
                    ...
                    if current_tile:  # 在地图上放置地皮
                        ...
                        tile_rect = tile_images[current_tile].get_rect(center=(mouse_x + camera_pos[0], mouse_y + camera_pos[1]))
    ...
    for tile, tile_rect in map_tiles:
        screen.blit(tile_images[tile], tile_rect.move(-camera_pos[0], -camera_pos[1]))

    # 显示当前选中的地皮预览，保持比例，中心位于鼠标位置
    if current_tile:
        mouse_x, mouse_y = pygame.mouse.get_pos()
        preview_image = tile_images[current_tile]
    ...
    for tile, tile_rect in map_tiles:
        thumb_rect = pygame.Rect(tile_rect.x * scale_factor, tile_rect.y * scale_factor, tile_rect.width * scale_factor, tile_rect.height * scale_factor)
